...
class SegmentationNN(nn.Module):
    ...
    def __init__(self, num_classes=23, hp=None):
        ...
        mobilenet.features[0][0] = nn.Conv2d(3, 32, kernel_size=3, stride=2, padding=1)
        ...
        self.upsample2 = nn.ConvTranspose2d(64, num_classes, kernel_size=4, stride=4, padding=0)
    ...
    def forward(self, x):
        ...
        x = self.features(x)
        x = self.conv(x)     # Reduce channel dimensions
        x = self.upsample1(x) # Upsample to intermediate resolution
        x = self.upsample2(x)
